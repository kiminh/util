...
private:
  using ModelDict = std::unordered_map<KeyType, ftrlentry>;
...
private:
  bool ParseLine(std::string& line, fea::instance& ins)
  {
    ...
    {
      ins.fea_vec.push_back((KeyType)(atol(fields[index].c_str())));
    }
    return true;
  }
  ...
  {
    ValueType sum = 0.0;
    std::vector<KeyType> fea_vec = ins.fea_vec;
    for(size_t index = 0;index < fea_vec.size();++index)
    {
      KeyType fea_index = fea_vec[index];
