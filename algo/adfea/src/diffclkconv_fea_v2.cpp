...
namespace fea
{
    ...
    bool diffclkconv_fea_v2::extract_fea(const record& record, fea_result& result)
    {
        string fea;
        ...
        {
            fea = "0_" + to_string(click_th1) + "_0";
        }else if(click < max_trust_click1){
            fea = to_string(conv) + "_" + to_string(click) + "_0";
        }else{
            int c1 = static_cast<int>(conv * 1.0/click * click_th1);

            if(cur_click < max_trust_click2 || pre_click < max_trust_click2)
                fea = to_string(c1) + "_" + to_string(click_th1) + "_x";
            else{
                ...
                fea = to_string(c1) + "_" + to_string(click_th1) + "_" + to_string(c2);
            }
        }
