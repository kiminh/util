...
def process(infile,nowapfile):
    ...
    for raw_line in open(infile):
        ...
        try:
            if "native_info" in meta_info:
                native_info = json.loads(meta_info["native_info"])
                if "image" in native_info:
                    image_list = native_info["image"]
                if "word" in native_info:
                    word_list = native_info["word"]
                if "native_config_id" in native_info:
        ...
        try:
            if "ext" in meta_info:
                ...
                try:
                    ...
                    if "native_adtype" in ext_info:
                        native_type = ext_info["native_adtype"]
                except:
                    pass
        except TypeError:
            pass
