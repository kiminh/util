...
class FM_FTRL_machine(object):
    ...
    def load_model(self,model_LR,model_FM):
        for raw_line in open(model_LR):
            line = raw_line.rstrip().split(",")
            self.w[int(line[0])] = float(line[1])

        for raw_line in open(model_FM):
            line = raw_line.rstrip().split(",")
            self.w_fm[int(line[0])] = [ float(weight)for weight in line[1:]]
    ...
    def predict_raw_p(self,x):
        ...
        for i in range(len_x):
            for j in range(i + 1, len_x):
                for k in range(self.fm_dim):
                    try:
                        raw_y += self.w_fm[x[i]][k] * self.w_fm[x[j]][k]
                    except KeyError:
                        pass
        return raw_y
    ...
    def predict_p(self,x):
        return 1. / (1. + exp(- max(min(self.predict_raw_p(x), 35.), -35.)))
...
def data_nohash(filePath, hashSize, hashSalt):
    for raw_line in open(filePath):
        line = raw_line.lower().rstrip("\r\n").split(" ")
        y = int(line[0])
        x = [int(term) for term in line[1:]]
        yield x,y,raw_line.lower().rstrip("\r\n")
...
def data(filePath, hashSize, hashSalt):
    ...
    for raw_line in open(filePath):
        ...
        for key in line[1:]:
            #tmp = item.split(":")
            #value = 1
            #index = abs(hash(hashSalt + key)) % hashSize + 1      # 1 is added to hash index because I want 0 to indicate the bias term.
            index = abs(hash(hashSalt + key)) % hashSize + 1      # 1 is added to hash index because I want 0 to indicate the bias term.
            x.append(index)
        yield x,y,raw_line.lower().rstrip("\r\n")
        #yield t, date, ID, x, y
