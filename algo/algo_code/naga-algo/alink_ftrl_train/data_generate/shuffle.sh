...
for i in `seq 1 7`;do
    ...
    done_file="/user/${USER_NAME}/ctr_space/ed_join_click/join/${day}/_SUCCESS"
    INPUTDIR="-input \"/user/$USER_NAME/ctr_space/ed_join_click/join/${day}/\" "${INPUTDIR} 
    loop_check $done_file 140 1
done

OUTPUTDIR="/user/ad_user/alink_train_data_week_sample"
done_file="/user/$USER_NAME/ctr_space/ed_join_click/join/${DATE}/_SUCCESS"
