...
if __name__ == '__main__':
    ...
    ed_df = spark.sql("select extra as ed_log, log_time as ed_time, reqid, adid, spam from dw.dw_usage_naga_dsp_ed_h where dt=%s or dt=%s" % (DATE, yestday))
    clk_df = spark.sql("select extra as click_log, log_time as click_time, reqid, adid, spam as tspam from dw.dw_usage_naga_dsp_click_h where dt=%s or dt=%s" % (DATE, yestday))
    ed_df.show(10, False)
