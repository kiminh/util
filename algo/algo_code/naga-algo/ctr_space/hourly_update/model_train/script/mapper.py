...
def hashstr(rawlog, nr_bins=10000001):
    #return pymurmur.hash(str(rawlog))%(nr_bins)+1
    return pymurmur.hash(str(rawlog))
...
fea_suffix_dict = dict((line.strip().split('\t')[0], line.strip().split('\t')[1]) for line in open('./script/features.list').readlines())
app_dict = parse_app_db('./script/app_db.dat')
plid_dict = load_plid_edclk('./script/plid_edclk.json')
#if len(sys.argv) == 2:
#    tag = sys.argv[1]
#else:
...
for line in sys.stdin:
    try:
        ...
        if plid in plid_dict and plid_dict[plid] > 0.5:
            continue
        req_style = ed_log.get("req_style", "")
        if req_style == "6" or req_style == "%06":
            continue
        ...
        for k,v in fea_dict.iteritems():
            if k not in fea_suffix_dict: 
                continue
            suffix = fea_suffix_dict[k]
            value = "%s\001%s:%s" % (k, suffix, v)
            ins_encode_list.append(value)
            item = stat_info[value]
            item.pv += 1
            item.click += click
