...
lock_ed_fo = threading.Lock()
lock_clk_fo = threading.Lock()
...
def process_message():
  while True:
    if not message_queue.empty():
      ...
      if not request_value.has_key('DSPED_LOG') and not request_value.has_key(
          'DSPCLICK_LOG'):
        continue
      ...
      if request_value.has_key('DSPED_LOG'):
        ...
        ed_acquired = lock_ed_fo.acquire()
        try:
          ed_fo.write("%s\n" % json.dumps(log))
        finally:
          if ed_acquired:
        ...
        log['log_time'] = log_time
        clk_acquired = lock_clk_fo.acquire()
        try:
          click_fo.write("%s\n" % json.dumps(log))
        finally:
          if clk_acquired:
...
def start_processor(number_of_processor):
  threads = list()
  for index in range(number_of_processor):
...
class PyKafka:
  ...
  def getConnect(self):
    ...
    self.consumer = topic.get_balanced_consumer(
        ...
        # 在consumer_group存在的情况下，设置此变量，表示从最新的开始取
        managed=True
  ...
  def init_file_fo(self):
    ...
    ed_output_file = "%s/ed/ed.%s.log" % (self.output_path, current_hour)
    click_output_file = "%s/click/click.%s.log" % (
    self.output_path, current_hour)
  ...
  def beginConsumer(self):
    ...
    for oneLog in self.consumer:
      ...
      if current_hour != hour:
        ...
        click_fo = open(click_output_file, "w")
        logging.info(
          "write into %s and %s" % (show_output_file, click_output_file))

      message_queue.put(oneLog.value)
