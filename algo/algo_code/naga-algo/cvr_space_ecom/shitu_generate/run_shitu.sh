...
echo $INPUTDIR
OUTPUTDIR="/user/${USER_NAME}/ocpc_ecom/model_train${JOB_TAG}/shitu/${DATE}/"

done_file="/user/${USER_NAME}/ocpc_ecom/click_join_trans/temp/${DATE}/_SUCCESS"
...
TASK_NAME="naga_cvr_ecom_generate_shitu_log"
...
function hadoop_run() {
    ...
    $HADOOP_HOME/bin/hadoop jar /usr/local/hadoop-2.6.3/share/hadoop/tools/lib/hadoop-streaming-2.6.3.jar \
        -libjars  /home/ad_user/ling.fang/script/tools/mr_plugins-1.0.jar \
        -files 'script' \
        -D map.output.key.field.separator='#' \
        -D num.key.fields.for.partition=1 \
...
ADFEA_DIR=/user/${USER_NAME}/ocpc_ecom/model_train${JOB_TAG}/shitu/adfea/
ADFEA_PATH=$ADFEA_DIR/${DATE}/
INS_DIR=/user/${USER_NAME}/ocpc_ecom/model_train${JOB_TAG}/shitu/ins/
INS_PATH=$INS_DIR/${DATE}/
STATINFO_DIR=/user/${USER_NAME}/ocpc_ecom/model_train${JOB_TAG}/shitu/statinfo/
STATINFO_PATH=$STATINFO_DIR/${DATE}/
PLANCLKTRANS_DIR=/user/${USER_NAME}/ocpc_ecom/model_train${JOB_TAG}/shitu/planclktrans/
PLANCLKTRANS_PATH=$PLANCLKTRANS_DIR/${DATE}/
SHITULOG_DIR=/user/${USER_NAME}/ocpc_ecom/model_train${JOB_TAG}/shitu/log/
...
$HADOOP_HOME/bin/hadoop fs -test -e $PLANCLKTRANS_PATH && $HADOOP_HOME/bin/hadoop fs -rm -r $PLANCLKTRANS_PATH
...
$HADOOP_HOME/bin/hadoop fs -mv $OUTPUTDIR/part-*-F $PLANCLKTRANS_PATH && \
