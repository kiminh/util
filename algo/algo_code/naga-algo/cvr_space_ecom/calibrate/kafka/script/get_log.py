...
output_path = "/home/work/ze.wei/naga-algorithm/cvr_space_ecom/calibrate/kafka/online_log/"
...
class PyKafka:
    ...
    TOPIC = "Cootek_ad_naga_dsp_click_transform"
    #BROKER_LIST = "cn-bdpkafka04.corp.cootek.com:9092,cn-bdpkafka05.corp.cootek.com:9092,cn-bdpkafka06.corp.cootek.com:9092"
    BROKER_LIST = "apd-kafka-cluster001.corp.cootek.com:9092,apd-kafka-cluster002.corp.cootek.com:9092,apd-kafka-cluster003.corp.cootek.com:9092,apd-kafka-cluster004.corp.cootek.com:9092,apd-kafka-cluster005.corp.cootek.com:9092"
    ...
    def getConnect(self):
        ...
        self.consumer = topic.get_balanced_consumer(
            consumer_group="cvr2online_20191219",
    ...
    def beginConsumer(self):
        ...
        for oneLog in self.consumer:
            ...
            click_trans_batch = []
            click_trans_tmp = json.loads(oneLog.value)
            if isinstance(click_trans_tmp, list):
                click_trans_batch = click_trans_tmp
            else:
                click_trans_batch.append(click_trans_tmp)

            for click_trans in click_trans_batch:
                url = click_trans['url']
                ...
                if request_value.has_key('DSPTRANSFORM_LOG'):
                    ...
                    industry = log.get('industry', '')
                    promoted_app = log.get('promoted_app', '')
                    cmod = log.get('cmod', '')
                    if (not industry.startswith('105')) or promoted_app != '' or cmod != '3':
                        continue
                    log['log_time'] = log_time
                    click_fo.write("%s\n" % json.dumps(log))
