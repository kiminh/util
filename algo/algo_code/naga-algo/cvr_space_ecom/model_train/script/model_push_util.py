...
class ModelPushUtil:
    def __init__(self, new_version, model_push_config):
        ...
        self.r_pipeline2 = None

        self.model_file = None
        self.model_meepo_dictname = None
        self.fealist_meepo_dictname = None
        self.cali_file_meepo_dictname = None
        self.fea_list = None
        self.calibrate_file = None
    ...
    def parse_config(self):
        with open(self.config_file) as f_in:
            for raw_line in f_in:
                ...
                elif key_strip == "redis_host2":
                    self.redis_host2 = value_strip
                elif key_strip == "redis_port2":
                    self.redis_port2 = value_strip
                ...
                elif key_strip == "cali_meepo_dictname":
                    self.cali_file_meepo_dictname = value_strip
                elif key_strip == "calibrate_file":
                    self.calibrate_file = value_strip
                else:
                    continue
    ...
    def init_redis_client(self):
        ...
        pool2 = redis.ConnectionPool(host=self.redis_host2,
                                    port=self.redis_port2, db=0)
        r2 = redis.StrictRedis(connection_pool=pool2)
        self.r_pipeline2 = r2.pipeline(transaction=False)
        #add test redis
    ...
    def parse_model_file(self):
        for raw_line in open(self.model_file):
            flds = raw_line.strip("\n\r ").split(" ")
            if len(flds) > 1 and float(flds[1]) != 0.:
                yield flds[0], flds[1]
            else:
                continue
    ...
    def push_calibrate_file(self):
        meepo_task = self.init_meepo_task(self.cali_file_meepo_dictname)
        for key, value in self.parse_cali_file():
            meepo_task.write_string(str(key), value)
        self.upload(meepo_task)

    def parse_cali_file(self):
        for raw_line in open(self.calibrate_file):
            line_sp = raw_line.strip("\n\r ").split('\t')
            if len(line_sp) != 2:
                continue
            promoted_app = line_sp[0]
            value = line_sp[1]
            yield promoted_app, value
    ...
    def delete_old_version(self, data_type):
        try:
            ...
            print sanpshot_version_path
            lines = []
            if os.path.exists(sanpshot_version_path):
                #stable_dt = (date.today() + timedelta(days = -3)).strftime("%Y%m%d")
                stable_dt = (datetime.now() + timedelta(hours = -10)).strftime("%Y%m%d%H%M%S")
                print stable_dt
                with open(sanpshot_version_path, 'r') as f_r:
                    lines = f_r.readlines()

            print lines
            online_version = lines[-1]
            with open(sanpshot_version_path, 'w') as f_w:
                for line in lines:
                    ver = line.strip('\n')
                    dt_str = int(ver)
                    if int(dt_str) < int(stable_dt) and dt_str != self.new_version \
                        and int(dt_str) != int(online_version):
                        print dt_str, stable_dt
                        self.meepo_writer.remove_snapshot("ad", meepo_name, int(ver))
                        self.meepo_writer.remove_snapshot("ad", 'ocpc2_fea_list', int(ver))
                        self.meepo_writer.remove_snapshot("ad", 'ocpc2_calibrate', int(ver))
                        continue
                    f_w.write("%s\n"%(ver))
                # add latest version 
                f_w.write("%s\n"%(self.new_version))
    ...
    def update_redis(self):
        try:
            ...
            self.r_pipeline2.set(self.redis_key, self.new_version)
            self.r_pipeline2.execute()
