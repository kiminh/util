...
for pb_app, value in clc_trans.items():
    ...
    if pb_app in app_back_dict:
        [hour_back_rate, day_back_rate] = app_back_dict[pb_app]
    ...
    if app_click < 200:
        ...
        global_app_diff = day_back_rate - (app_pcvr_cal / (app_trans+1))
...
for pb_app, value in slot_error.items():
    ...
    theta_exp[pb_app] = min(max(theta_exp[pb_app], theta_min), theta_max)
print "==================================global error=============================="
...
out_path='./data_exp/theta_%s.json' % sys.argv[3]
out_path1='./data_exp/result_%s.json' % sys.argv[3]
