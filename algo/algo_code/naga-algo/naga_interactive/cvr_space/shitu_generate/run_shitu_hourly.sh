...
INPUTDIR="-input \"/user/$USER_NAME/naga_interactive/ocpc/click_join_trans${JOB_TAG}/join_path/$VERSION\" "${INPUTDIR}
OUTPUTDIR="/user/${USER_NAME}/naga_interactive/ocpc/hour_model_train${JOB_TAG}/shitu/${DATE}/"
done_file="/user/$USER_NAME/naga_interactive/ocpc/click_join_trans${JOB_TAG}/join_path/$VERSION"
loop_check $done_file 140 1

OWNER_INFO="$USER_NAME@cootek.cn"
TASK_NAME="naga_interactive_cvr_generate_shitu_log (hourly)"
...
ADFEA_DIR=/user/${USER_NAME}/naga_interactive/ocpc/hour_model_train${JOB_TAG}/shitu/adfea/
ADFEA_PATH=$ADFEA_DIR/${DATE}/
INS_DIR=/user/${USER_NAME}/naga_interactive/ocpc/hour_model_train${JOB_TAG}/shitu/ins/
INS_PATH=$INS_DIR/${DATE}/
STATINFO_DIR=/user/${USER_NAME}/naga_interactive/ocpc/hour_model_train${JOB_TAG}/shitu/statinfo/
STATINFO_PATH=$STATINFO_DIR/${DATE}/
ACCEPT_PKG_DIR=/user/${USER_NAME}/naga_interactive/ocpc/hour_model_train${JOB_TAG}/shitu/acceptpkg/
ACCEPT_PKG_PATH=$ACCEPT_PKG_DIR/${DATE}/
APPCLKTRANS_DIR=/user/${USER_NAME}/naga_interactive/ocpc/hour_model_train${JOB_TAG}/shitu/appclktrans/
...
$HADOOP_HOME/bin/hadoop fs -mv $OUTPUTDIR/part-*-D $ACCEPT_PKG_PATH && \
$HADOOP_HOME/bin/hadoop fs -touchz $ACCEPT_PKG_PATH/_SUCCESS
$HADOOP_HOME/bin/hadoop fs -mv $OUTPUTDIR/part-*-E $APPCLKTRANS_PATH && \
