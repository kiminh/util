...
hour = day[8:10]
pkg_cali_dict = json.load(open(sys.argv[1]))
plan_cali_dict = json.load(open(sys.argv[2]))

r = redis.Redis(host='ali-cache07.corp.cootek.com', port=16359)
prefix = 'dsp_interactive_ocpc_discount'
...
with r.pipeline(transaction=False) as p, r1.pipeline(transaction=False) as p1:
    ...
    """
    for pkg, discount in pkg_cali_dict.items():
        dict_name = '%s:%s' % (prefix, pkg)
