...
HADOOP2_BIN="/usr/local/hadoop-ha_new2/bin/hadoop"
ROOT_PATH="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"

function alarm() {
    python $ROOT_PATH/alarm.py "$1" "$2"
}

function email() {
    python $ROOT_PATH/email_sender.py "$1" "$2"
}
...
function hadoop2_check_ready() {
   $HADOOP2_BIN fs -test -e $file && ready=1 || \
   { ready=0;}
}
...
function redis_check_ready() {
    res_code=`python $ROOT_PATH/get_flag.py $file`
