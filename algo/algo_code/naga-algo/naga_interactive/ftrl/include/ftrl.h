...
private:
  using ModelDict = std::unordered_map<fea::KeyType, ftrlentry>;
...
private:
  bool ParseLine(std::string& line, fea::instance& ins)
  {
    ...
    {
      struct fea::fea_item fi;
      std::vector<std::string> items;
      util::str_util::split(fields[index], ":", items);
      if(items.size() == 1){
        fi.fea_index = (fea::KeyType)(atoi(items[0].c_str()));
        fi.fea_val = 1;
      }else{
        fi.fea_index = (fea::KeyType)(atoi(items[0].c_str()));
        fi.fea_val = (float)(atof(items[1].c_str()));
      }
      ins.fea_vec.push_back(fi);
    }   
    return true;
  }
  ...
  {
    ValueType sum = 0.0;
    std::vector<fea::fea_item> fea_vec = ins.fea_vec;
    for(size_t index = 0;index < fea_vec.size();++index)
    {
      fea::fea_item fi = fea_vec[index];
      if(model.count(fi.fea_index))
        sum += (model[fi.fea_index].w) * fi.fea_val;
    }
    return Sigmoid(sum);
  }
