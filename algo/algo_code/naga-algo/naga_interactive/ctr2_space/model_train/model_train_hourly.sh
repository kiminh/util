...
if [[  -e $batch_model_path/daily_model/lr_model.dat.${file_day_flag} ]];then
...
for file in `ls $root_path/../shitu_generate/shitu/shitu_[0-9]* | tac`;do
    echo $file
...
model_file="./model/lr_model_online.dat"
if [[ $is_batch_model_ready -eq 1 ]];then    
    model_file=$batch_model_path/daily_model/lr_model.dat.${file_day_flag}
fi
new_model_file="./model/lr_model_online.dat.new"

./bin/ftrl ${incre_shitu} alpha=0.01 beta=1 l1_reg=1 l2_reg=0. \
           model_out=${new_model_file} save_aux=1 is_incre=1 model_in=${model_file}
[ $? -ne 0 ] && { alarm "Naga Interactive CTR2 Model Hourly Update" "ftrl model train error!"; exit 1; }

if [[ $is_batch_model_ready -eq 1 ]];then    
    mv $batch_model_path/daily_model/lr_model.dat.${file_day_flag} $batch_model_path/daily_model_bk/
    mv ./model/lr_model_online.dat ./model_bk/"lr_model_online.dat".${VERSION}
    mv ${new_model_file} ./model/lr_model_online.dat
else
    mv ${model_file} ./model_bk/"lr_model_online.dat".${VERSION}
    mv ${new_model_file} ${model_file}
fi
python script/model_push_util.py $VERSION conf/model_push.conf
[ $? -ne 0 ] && { alarm "Naga Interactive CTR2 Model Hourly Update" "model push online $VERSION error!"; exit 1}
exit 0
