...
if __name__ == '__main__':
    ...
    spark = SparkSession\
        .builder \
        .appName("naga DSP interactive: ed_join_click_%s" % (cur_time))\
        .enableHiveSupport()\
    ...
    first_ed_df = spark.sql("select extra as first_ed_log, log_time as first_ed_time, reqid from dw.dw_usage_ad_naga_interactive_first_ed_d where dt=%s and (hour=%s or hour=%s)" % (day, cur_time[8:], pre_time[8:]))
    first_clk_df = spark.sql("select extra as first_click_log, log_time as first_click_time, reqid as treqid from dw.dw_usage_ad_naga_interactive_first_click_d where dt=%s and (hour=%s or hour=%s)" % (day, cur_time[8:], pre_time[8:]))
    second_ed_df = spark.sql("select extra as second_ed_log, log_time as second_ed_time, reqid as treqid from dw.dw_usage_ad_naga_interactive_second_ed_d where dt=%s and (hour=%s or hour=%s)" % (day, cur_time[8:], pre_time[8:]))
    second_clk_df = spark.sql("select extra as second_click_log, log_time as second_click_time, reqid as treqid, number from dw.dw_usage_ad_naga_interactive_second_click_d where dt=%s and (hour=%s or hour=%s)" % (day, cur_time[8:], pre_time[8:]))
    ...
    cur_ed_join_clk = ed_join_clk.filter(ed_join_clk.first_ed_time.substr(1, 10) == cur_time) # 20191012
    pre_ed_join_clk = ed_join_clk.filter(ed_join_clk.first_ed_time.substr(1, 10) == pre_time)
