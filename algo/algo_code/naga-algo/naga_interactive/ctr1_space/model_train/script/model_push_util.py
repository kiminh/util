...
class ModelPushUtil:
    ...
    def push_model(self):
        ...
        #meepo_task.write_number("1430112322", -0.450328)
    ...
    def delete_old_version(self, data_type):
        try:
            ...
            with open(sanpshot_version_path, 'w') as f_w:
                for line in lines:
                    ver = int(line.strip('\n'))
                    if ver < int(stable_dt) and ver != self.new_version and ver != int(online_ver):
                        self.meepo_writer.remove_snapshot("ad", meepo_name, int(ver))
                        self.meepo_writer.remove_snapshot("ad", self.fealist_meepo_dictname, int(ver))
