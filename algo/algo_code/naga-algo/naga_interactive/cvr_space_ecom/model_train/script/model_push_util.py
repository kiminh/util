...
class ModelPushUtil:
    ...
    def init_redis_client(self):
        ...
        #add test redis
        test_pool = redis.ConnectionPool(host="ali-cache01.corp.cootek.com",
                                         port="17131", db=0)
    ...
    def upload(self, task):
        try:
            ...
            while task.if_can_commit() == False:
                ...
                if new_precent == int(task.precent()):
            ...
            print("upload success: group=%s name=%s version=%s time:%is" % \
                    (str(task.group()), task.name(), task.version(), ed_time-start_time))
        except Exception as e:
    ...
    def delete_old_version(self, data_type):
        try:
            ...
            if os.path.exists(sanpshot_version_path):
                stable_dt = (datetime.now() + timedelta(hours = -6)).strftime("%Y%m%d%H%M%S")
                with open(sanpshot_version_path, 'r') as f_r:
                    lines = f_r.readlines()

            try:
                self.r_pipeline.get(self.redis_key)
                ret = self.r_pipeline.execute()
                online_ver = ret[0]
                print("get redis model version[%s]" % online_ver)
            except Exception as e:
                print("get redis model version occured exception: %s" % e)
                exit(1)
            ...
            with open(sanpshot_version_path, 'w') as f_w:
                for line in lines:
                    ver = line.strip('\n')
                    dt_str = int(ver)
                    if dt_str < int(stable_dt) and dt_str != self.new_version \
                        and dt_str != int(online_ver):
                        print dt_str, stable_dt
                        self.meepo_writer.remove_snapshot("ad", meepo_name, int(ver))
                        self.meepo_writer.remove_snapshot("ad", 'ocpc_fea_list', int(ver))
                        self.meepo_writer.remove_snapshot("ad", 'ocpc_calibrate', int(ver))
                        continue
                    f_w.write("%s\n"%(ver))
                # add latest version 
                f_w.write("%s\n"%(self.new_version))
