...
SHITU_INS="/user/${USER_NAME}/naga_interactive/ocpc_ecom/hour_model_train${JOB_TAG}/shitu/ins/$TODAY"
$HADOOP_HOME/bin/hadoop fs -cat $SHITU_INS/* > $LOCAL_SHITU_INS/shitu_ins
[ $? -ne 0 ] && { alarm "Nage Interactive DSP Ecom CVR Model" "$SHITU_INS not exist"; exit 1; }

shuf ${LOCAL_SHITU_INS}/shitu_ins -o ${LOCAL_SHITU_INS}/shitu_ins.shuffle

MODEL_FILE="./daily_model/lr_model.dat.$DATE"
[ ! -e $MODEL_FILE ] && alarm "Nage Interactive DSP Ecom CVR Model" "daily model $DATE not exist" && exit 1
...
[ $? -ne 0 ] && { alarm "Nage Interactive DSP Ecom CVR Model" "model train $VERSION error!"; exit 1; }
