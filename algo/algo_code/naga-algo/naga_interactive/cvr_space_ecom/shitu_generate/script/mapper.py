...
for line in sys.stdin:
    try:
        ...
        planid = ed_log.get('planid', UNK_FEA)
        fea_dict['date'] = timestamp[0:8]

        if int(fea_dict['date']) < 20200321 and planid == 'P81569n3zb4drdkw':
            continue
        if int(fea_dict['date']) < 20200322 and planid == 'P1070omx0gcezyc2':
            continue 
        ...
        fea_dict['make_model_planid'] = concat_str(fea_dict['make_model'], fea_dict['planid'])
        fea_dict['iswifi_planid'] = concat_str(fea_dict['iswifi_planid'], fea_dict['planid'])
        ...
        if planid != UNK_FEA:
            if planid not in plan_clicktrans:
                plan_clicktrans[planid] = {'click': 0, 'trans': 0}
            plan_clicktrans[planid]['click'] += 1
            plan_clicktrans[planid]['trans'] += int(is_trans)
    except Exception as e:
...
for key, value in plan_clicktrans.items():
    print "%s\002planclktrans\t%s\t%s" % (key, value['click'], value['trans'])
