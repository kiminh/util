...
if __name__ == '__main__':
    ...
    clk_df = spark.sql("select extra as click_log, log_time as click_time, reqid, adid, spam, planid, adpcvr as adpcvr_tmp, pcvr_cal as pcvr_cal_tmp, is_ocpc from dw.dw_usage_naga_dsp_click_h where dt > {} and ad_style = 8 and promoted_app = '' and get_json_object(extra, '$.industry') like '105%' and spam = 0".format(DATE))
    trans_df = spark.sql("select extra as trans_log, log_time as trans_time, reqid, adid, spam from dw.dw_usage_naga_dsp_transform_h where dt > {} and ad_style = 8 and promoted_app = '' and get_json_object(extra, '$.industry') like '105%' and spam = 0 and event_type = '35'".format(DATE))
