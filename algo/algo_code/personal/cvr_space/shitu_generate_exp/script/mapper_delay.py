...
for line in sys.stdin:
    try:
        ...
        trans_time = time.strftime("%Y%m%d000000", time.localtime())
        ...
        if mode == 'online': 
            if 'trans_log' in ld:
                ...
                if '0' in events:
                    is_trans = 1
                    idx = events.index("0")
                    trans_time = '%s %s' % (ld['trans_log']['rawlog'][idx]['date'], 
                        ld['trans_log']['rawlog'][idx]['time'])
                    trans_ta = time.strptime(trans_time, "%Y-%m-%d %H:%M:%S")
        else:
            is_trans = ld['trans_log']
        click_ta = time.strptime(ld['click_time'], "%Y%m%d%H%M%S")
        delay_time = int(time.mktime(trans_ta)) - int(time.mktime(click_ta))
        delay_time = int(delay_time / (60*60))
        ...
        print "%s\002ins\t%s\t%s\t%s" % (fea_dict['ifa'], ' '.join(str(x) for x in ins_encode_list), str(is_trans), delay_time)
