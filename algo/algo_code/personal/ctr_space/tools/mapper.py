...
fea_suffix_dict = {}
for line in open('./features.list').readlines():
    if line[0] == '#':
        continue
    flds = line.strip("\n\r ").split('\t')
    fea_name = flds[0]
...
for line in sys.stdin:
    try:
        ...
        #if plid in plid_pwh:
        #    fea_dict['pw'] = plid_pwh[plid]['pw']
        #    fea_dict['ph'] = plid_pwh[plid]['ph']
        ...
        for fea in fea_suffix_dict:
            ...
            if value not in stat_info:
                stat_info[value] = 0
            stat_info[value] += 1
        ins_encode_list.append("%s" % ('9999:1'))
...
stat_info_sort = sorted(stat_info.items(), key=lambda d:d[0], reverse=False)
with open('fea_dict_ed.txt', 'w') as f_out:
    for key, value in stat_info_sort:
        f_out.write('%s\t%s\n' % (key, value))
f_out.close()
#json.dump(stat_info, open('fea_dict_ed.json', 'w'), indent=4)
