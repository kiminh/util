...
function hadoop_run() {
    ...
    $HADOOP_HOME/bin/hadoop jar /usr/local/hadoop-2.6.3/share/hadoop/tools/lib/hadoop-streaming-2.6.3.jar \
        ...
        -Dmapreduce.job.queuename=root.ad-root.etl.dailyetl.veryhigh \
...
$HADOOP_HOME/bin/hadoop fs -rm -r $OUTPUTDIR && \
{ echo "task[$DATE] success!"; exit 0;} || { echo "hadoop mv hdfs file failed!"; alarm "ED_JOIN_CLICK" "task[${TASK_NAME}_${DATE}] failed...."; exit -1;}
