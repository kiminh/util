...
class ModelPushUtil:
    ...
    def parse_config(self):
        with open(self.config_file) as f_in:
            for raw_line in f_in:
                ...
                elif key_strip == "gp_crawl":
                    self.gp_crawl = value_strip
                elif key_strip == "key_prefix":
                    self.key_prefix = value_strip
        ...
        for raw_line in open(self.fea_list):
            ...
            for item in line_sp:
                ...
                if item_sp[0] == "fea_name":
                    fea_name = item_sp[1]
            if slot == "" or fea_name == "":
                print("fea list format error!")
                exit(1)
            yield fea_name, slot
    
    def push_gp_crawl(self):
        return None
    
    def delete_old_version(self, data_type):
        try:
            meepo_name = "%s" % (data_type)
            sanpshot_version_path = "./version/%s.ver" % (meepo_name)
            if not os.path.exists(sanpshot_version_path):
                pass
            stable_dt = (date.today() + timedelta(days = -2)).strftime("%Y%m%d")
            print stable_dt
    ...
    def update_redis(self):
        try:
            ...
            print('redis addr[%s], redis port[%s]' % (self.redis_host, self.redis_port))
            ...
            pool = redis.ConnectionPool(host=self.redis_host,
                                    port=self.redis_port, db=0)
            r = redis.StrictRedis(connection_pool=pool)
            r_pipeline = r.pipeline(transaction=False) 
            r_pipeline.set(self.redis_key, self.new_version)
            r_pipeline.execute()
        except Exception as e:
            print(e)
            exit(1)
