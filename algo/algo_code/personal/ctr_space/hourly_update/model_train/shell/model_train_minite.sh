...
cur_file_time_flag=$(date -d "0 hour"  +%Y%m%d%H)
pre_file_time_flag=$(date -d "-2 hour"  +%Y%m%d%H)
echo $file_time_flag
local_log=/home/ling.fang/ctr_space/hourly_update/log/
...
cat $local_log/ed/ed.${cur_file_time_flag}.log > ${incre_path}/ed.log
cat $local_log/ed/ed.${file_time_flag}.log >> ${incre_path}/ed.log
cat $local_log/click/click.${cur_file_time_flag}.log > ${incre_path}/click.log
cat $local_log/click/click.${file_time_flag}.log >> ${incre_path}/click.log

hour_shitu_output=${incre_path}/h_incre_shitu_tmp

#补当前小时的部分日志
python script/join_click.py ${incre_path}/ed.log $local_log/click/click.${pre_file_time_flag}.log \
     ${incre_path}/click.log ${tmp_shitu_output}
