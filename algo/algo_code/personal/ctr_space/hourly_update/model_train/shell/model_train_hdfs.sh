...
for file in `$HADOOP_HOME fs -ls $data_path/* | tac`;do
    echo $file
...
./ftrl/bin/ftrl ${incre_shitu}.shuffle alpha=0.01 beta=1 l1_reg=1 l2_reg=0. \
...
./ftrl/bin/ftrl ${tmp_shitu_output}.shuffle alpha=0.01 beta=1 l1_reg=1 l2_reg=0. \
           model_in=${model_file} model_out=${model_online_file} save_aux=0 is_incre=1
if [[ $? -ne 0 ]];then
  echo "ftrl train error!!"
  exit 1
fi
