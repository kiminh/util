...
JOIN_PATH="/user/$USER_NAME/ctr_space/ed_join_click_exp_hour/join/${DATE}${HOUR}"
YESTDAY_JOIN_PATH="/user/$USER_NAME/ctr_space/ed_join_click_exp_hour/join/${PREDATE}${PREHOUR}"
UNJOIN_PATH="/user/$USER_NAME/ctr_space/ed_join_click_exp_hour/unjoin/${DATE}${HOUR}"
...
sparksubmit="${SPARK_HOME}/bin/spark-submit \
                 --master yarn \
                 --queue root.ad-root.etl.dailyetl.veryhigh \
                 --num-executors 50 \
                 --executor-memory 8g \
                 --driver-memory 12g \
