    ...
    def delete_old_version(self, data_type):
        try:
            ...
            if os.path.exists(sanpshot_version_path):
                stable_dt = (datetime.now() + timedelta(hours = -2)).strftime("%Y%m%d%H%M%S")
                print stable_dt
            ...
            with open(sanpshot_version_path, 'w') as f_w:
                for line in lines:
                    ver = line.strip('\n')
                    dt_str = int(ver)
                    #if dt_str < int(stable_dt+'00') and dt_str != self.new_version and dt_str != int(online_ver):
                    if dt_str < int(stable_dt) and dt_str != self.new_version and dt_str != int(online_ver):
                        self.meepo_writer.remove_snapshot("ad", meepo_name, int(ver))
                        self.meepo_writer.remove_snapshot("ad", 'ctr_fea_list', int(ver))
                        print "delete old model version: %s" % ver
                        continue
                    f_w.write("%s\n"%(ver))
                # add latest version 
                f_w.write("%s\n"%(self.new_version))
