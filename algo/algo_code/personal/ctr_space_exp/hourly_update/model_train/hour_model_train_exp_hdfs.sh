...
alarm "Nage DSP Hour Model" "[$VERSION] exp ftrl ctr model train success! $bias_res"
python script/model_push_util.py $VERSION conf/model_push.conf
if [[ $? -ne 0 ]];then
    alarm "Nage DSP Hour Model" "exp model push online $VERSION error!"   
    exit 1
fi
exit 0
...
#python utils/sms_sender.py "[$VERSION] ftrl ctr model train success! $bias_res"
alarm "Nage DSP Hour Model" "[$VERSION] exp ftrl ctr model train success! $bias_res"
exit 0
