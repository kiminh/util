...
if [[ $? -ne 0 ]];then
    alarm "Nage DSP CTR Calibrate" "stat_adstyle_edclk_exp $file_time_flag error!"
    exit 1
fi

[[ ! -d ./data ]] && mkdir data
$HADOOP_HOME/bin/hadoop fs -cat /user/ad_user/ctr_space/calibrate_exp/$file_time_flag/* > data/ed_click_$file_time_flag.json
if [[ $? -ne 0 ]];then
    alarm "Nage DSP CTR Calibrate" "hadoop cat data $file_time_flag error!"
    mv log.txt log/log.$file_time_flag.txt.error
    exit 1
fi
...
mv log.txt log/log.$file_time_flag.txt
