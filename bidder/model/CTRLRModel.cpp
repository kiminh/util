...
CTRLRModel::CTRLRModel(int id)
    : CTRModelBase("CTRLRModel", id)
    , _ftrl_model_mgr_t(nullptr)
    , using_gbdt(false)
    , _gbdt_model_mgr_t(nullptr)
{
}

CTRLRModel::~CTRLRModel()
{
    if (_ftrl_model_mgr_t != nullptr) {
        delete _ftrl_model_mgr_t;
    }
    _ftrl_model_mgr_t = nullptr;
...
int CTRLRModel::init(const Json::Value &parameters)
{
    ...
    if (using_gbdt) {
        ...
        if (_gbdt_model_mgr_t == nullptr) {
            throw ML::Exception("New FMFMFTRL model Mgr error!");
        }
...
int CTRLRModel::model_reloader()
{
    if (_ftrl_model_mgr_t == nullptr) {
        return -1;
    }
    if (_ftrl_model_mgr_t->need_reload() == 1) {
        if (_ftrl_model_mgr_t->reload() < 0) {
            return -1;
        }
    }
    ...
    std::vector<uint32_t> & feas)
{
    ...
    FtrlModel *ftrl_model = _ftrl_model_mgr_t->get_content();
    if (ftrl_model == nullptr) {
        cerr << "get ftrl error" << endl;
        return 0.0;
    }
    float pctr = ftrl_model->score(_ad_instance, belta_bias);
    ...
    if (using_gbdt) {
        ...
        if (gbdt_model != nullptr) {
            ...
            pctr = ftrl_model->score(_ad_instance, belta_bias);
            cout << "get pctr: " << pctr << endl;
