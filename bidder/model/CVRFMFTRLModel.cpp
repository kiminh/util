...
CVRFMFTRLModel::CVRFMFTRLModel(int id)
    : CVRModelBase("CVRFMFTRLModel", id)
...
int CVRFMFTRLModel::init(const Json::Value &parameters)
{
...
int CVRFMFTRLModel::model_reloader()
{
    if (_fmftrl_mgr_t == nullptr) {
        return -1;
    }
...
double CVRFMFTRLModel::compute_cvr(
    ...
    std::vector<uint32_t> & feas)
{
    ...
    double pcvr = fmftrl_model->score(_ad_instance, belta_bias);
    ...
    if (using_gbdt) {
        ...
        if (gbdt_model != nullptr) {
            ...
            pcvr = fmftrl_model->score(_ad_instance, belta_bias);
