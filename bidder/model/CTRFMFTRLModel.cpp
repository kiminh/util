...
CTRFMFTRLModel::CTRFMFTRLModel(int id)
    : CTRModelBase("CTRFMFTRLModel", id)
...
int CTRFMFTRLModel::init(const Json::Value &parameters)
{
...
int CTRFMFTRLModel::model_reloader()
{
    if (_fmftrl_mgr_t == nullptr) {
        return -1;
    }
...
double CTRFMFTRLModel::compute_ctr(
    ...
    std::vector<uint32_t> & feas)
{
    ...
    double pctr = fmftrl_model->score(_ad_instance, belta_bias);
    ...
    if (using_gbdt) {
        ...
        if (gbdt_model != nullptr) {
            ...
            pctr = fmftrl_model->score(_ad_instance, belta_bias);
