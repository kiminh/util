...
CVRLRModel::CVRLRModel(int id)
    : CVRModelBase("CVRLRModel", id)
...
int CVRLRModel::init(const Json::Value &parameters)
{
    ...
    std::vector<uint32_t> & feas)
{
    ...
    float pcvr = ftrl_model->score(_ad_instance, belta_bias);
    ...
    if (using_gbdt) {
        ...
        if (gbdt_model != nullptr) {
            ...
            pcvr = ftrl_model->score(_ad_instance, belta_bias);
