...
CVRModelBase::CVRModelBase(string name, int id)
    : model_name(name)
    , model_id(id)
{
    for (int i = 0; i < threadNum; ++i) {
...
const string &CVRModelBase::get_model_name()
{
    return model_name;
}

int CVRModelBase::init(const Json::Value &parameters)
{
...
double CVRModelBase::compute_cvr(
...
int CVRModelBase::make_br_fields(shared_ptr <BidRequest> br, AdSpot &spot, vector <string> &extract_fields)
{
    //clear fields
    //extract_fields
...
int CVRModelBase::make_ad_fields(
    const BidderCreative &bidCreative,
    const Json::Value &augmentation,
    vector <string> &extract_fields)
{
    ...
    total = static_cast<int>(std::pow(log2(total+1.0), 2));
    extract_fields.push_back(to_string(total));
    ...
    vector <string> &extract_fields)
{
    ...
    string app_bundle;
    if (br->site) {
        app_bundle = br->site->name.rawString();
        extract_fields.push_back(br->site->id.toString());
        extract_fields.push_back(br->site->name.rawString());
        extract_fields.push_back(br->site->domain.rawString());
        if (br->site->cat.size() > 0) {
        ...
        app_bundle = br->app->name.rawString();
        extract_fields.push_back(br->app->id.toString());
        extract_fields.push_back(br->app->name.rawString());
        extract_fields.push_back(br->app->domain.rawString());
        if (br->app->cat.size() > 0) {
    ...
    extract_fields.push_back(bidCreative.format());
    extract_fields.push_back(to_string(bidCreative.ad_type()));
    extract_fields.push_back(to_string(bidCreative.tracking_type()));
...
int CVRModelBase::model_reloader()
{
    return 0;
}

int CVRModelBase::extract_br(std::shared_ptr <BidRequest> br, AdSpot &spot, int threadId)
{
