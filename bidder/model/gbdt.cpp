...
{
    if (trees != NULL) {
        for (int index = 0; index < tree_num; ++index) {
            release(trees[index]);
        }
        free(trees);
        trees = NULL;
...
int gbdt::get_leaf_index(fea::instance &ins, int index)
{
    int pos = 1;
...
void gbdt::get_trees_leaf_index(fea::instance &ins, std::vector <uint32_t> &index)
{
    int tree_leaf = 0;
    for (int i = 0; i < tree_num; ++i) {
        int pos = get_leaf_index(ins, i);
        index.push_back(pos + tree_leaf);
        tree_leaf += leaf_set[i].size();
    }
}

double gbdt::one_tree_predict_line(fea::instance &ins, int index)
{
    tree *root = trees[index];
    while (!root->is_leaf) {
        if (ins.is_missing(root->split)) {
...
void gbdt::print_leafs()
{
    ...
    for (int index = 0; index < this->tree_num; ++index) {
        std::cout << "trees[" << index << "]:" << " ";
        for (unsigned j = 0; j < leaf_set[index].size(); j++) {
            std::cout << leaf_set[index][j] << " ";
        }
        std::cout << std::endl;
    }
}
int gbdt::get_number_of_leafs()
{
    int sum_leafs = 0;
