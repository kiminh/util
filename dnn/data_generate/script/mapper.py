...
for line in open("./script/features.list"):
    if '#' in line:
        continue
    fea_name, fea_code = line.strip().split('\t')
...
for line in sys.stdin:
    try:
        ...
        fea_dict['applist'] = '_'.join(ld.get('applist', ['unk'])) #str(ld.get('applist', UNK_FEA))
        ...
        output_list = [is_trans]
        randint = random.randint(0, sys.maxint)
        for fea in fea_list:
            output_list.append(fea_dict[fea])
        print "%s\t%s" % (randint, ",".join(output_list))
