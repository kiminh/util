...
for i in `seq 1 7`;
do
    day=`date -d "-$i day 20190601" +%Y%m%d`
    days+=$day","
done
days+="}"
#days=$1
...
DES_PATH="/user/ling.fang/1/2/3/4/5/automated_targeting/usage_daily/$day_timestamp/"
...
$sparksubmit ${JOB_PATH}/extract_appusage_feature_ut.py $USER_APPLIST_DATA $APPUSAGE_PATH $APPDB_PATH $IDF2GAID $DES_PATH
