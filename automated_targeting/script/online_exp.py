...
if __name__ == '__main__':
    ...
    obj_pname_df = spark.read.txt(obj_pkg_path)
    obj_pname_df = obj_pname_df.withColumnRenamed('value', 'package_name')

    obj_pname_df = pkg_si_model.transform(obj_pname_df)
    pkg_index = obj_pname_df.select(obj_pname_df.pkg_index.cast('int'), 'package_name')
    pkg_index.show()
    ...
    pkg_vector = pkg_vector_df.select('features').collect()
    for i, j in pkg_vector:
